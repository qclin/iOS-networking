...
class MovieDetailViewController: UIViewController {
    ...
    @IBAction func toggleFavorite(_ sender: AnyObject) {
        ...
        /* 2/3. Build the URL, Configure the request */
        let request = NSMutableURLRequest(url: appDelegate.tmdbURLFromParameters(methodParameter as [String: AnyObject], withPathExtension: "/account/\(appDelegate.userID!)/favorite"))
        ...
        let task = appDelegate.sharedSession.dataTask(with: request as URLRequest){ (data, response, error) in
            ...
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
                print("request return a status code other than 2xx!")
                return
            }
            ...
            print("post to Favorites \(tmdbStatusCode) in \(parsedResult)")
            // add to favorite
